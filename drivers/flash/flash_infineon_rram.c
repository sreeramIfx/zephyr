...
static int ifx_rram_erase(const struct device *dev, off_t offset, size_t size)
{
	...
	static uint8_t buffer[CY_RRAM_BLOCK_SIZE_BYTES];
	...
	while (remaining > 0 && status == CY_RRAM_SUCCESS) {
		size_t chunk = MIN(remaining, sizeof(buffer));

		status = Cy_RRAM_NvmWriteByteArray(cfg->base, erase_offset, buffer, chunk);
		erase_offset += chunk;
		remaining -= chunk;
	}
	k_mutex_unlock(&dev_data->lock);
