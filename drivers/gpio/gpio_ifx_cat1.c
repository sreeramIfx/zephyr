...
#if (CONFIG_SOC_FAMILY_INFINEON_CAT1C)
#define __INTR_PRIORITY(n)

#if (CONFIG_CPU_CORTEX_M0PLUS)
#define __ENABLE_INT(n)
#else
#define __ENABLE_INT(n) ENABLE_SYS_INT(n, gpio_isr_handler);
#endif

#else
#define __INTR_PRIORITY(n) .intr_priority = DT_INST_IRQ_BY_IDX(n, 0, priority),
